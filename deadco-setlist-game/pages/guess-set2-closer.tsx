...
import { getGuestEmail, getSet2CloserWinnerStatus, storeSet2CloserWinnerStatus } from '@/lib/guestHelpers';
...
const GuessSet2CloserPage = () => {
  ...
  const prizeInfo = [
   ...
   useEffect(() => {
     ...
     const handleSet2CloserSubmission = async (playMode: string, amount?: number) => {
      ...
      const { data: setlistData, error: setlistError } = await supabase
        .from('setlists')
        .select('set2_closer, show_opens_at, winner_decision_time')
      ...
      const actualSong = setlistData.set2_closer?.trim().toLowerCase();
      ...
      alert(`Submitting your guess for "${selectedSong}"... "${actualSong}"`);
      // 🎮 Determine winner
      if (playMode === 'fun') {
        if (now >= showOpensAt) {
          isUserWinner = actualSong === guessedSong;
          setIsWinner(isUserWinner);
          storeSet2CloserWinnerStatus(isUserWinner);
        } else {
          setIsWinner(null);
          storeSet2CloserWinnerStatus(null);
        }
      } else {
        if (now >= winnerDecisionTime) {
          const status = actualSong === guessedSong;
          setIsWinner(status);
          storeSet2CloserWinnerStatus(status);
          isUserWinner = status;
        } else {
          const localStatus = getSet2CloserWinnerStatus();
      ...
      if (insertError) {
        ...
        alert('❌ There was a problem submitting your Set 2 Closer prediction.');
        return;
      }
  ...
  useEffect(() => {
    ...
    useEffect(() => {
      const winner = getSet2CloserWinnerStatus();
...
/>
        ...
        <div className="mt-8 mb-8 perspective-1500 rotateX-12">
            <FourWaysToPlay
              ...
              onSubmissionClick={handleSet2CloserSubmission}
              gameType="set2Closer prediction"
