  ...
  useEffect(() => {
    const fetchSongs = async () => {
      try {
        ...
        // Only add the song after Drums/Space (first song after "Drums" or "Space")
            ...
            setlist.sets.set.forEach((set: any) => {
              if (set.song?.length) {
                for (let i = 0; i < set.song.length - 1; i++) {
                  if (
                    set.song[i]?.name &&
                    (set.song[i].name.toLowerCase().includes('drums') ||
                      set.song[i].name.toLowerCase().includes('space'))
                  ) {
                    if (set.song[i + 1]?.name) songSet.add(set.song[i + 1].name);
                  }
                }
              }
            });
