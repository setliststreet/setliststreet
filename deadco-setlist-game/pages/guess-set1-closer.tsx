...
import { getGuestEmail , getSet1CloserWinnerStatus, storeSet1CloserWinnerStatus } from '@/lib/guestHelpers';
...
const GuessSet1CloserPage = () => {
  ...
  useEffect(() => {
    const winner = getSet1CloserWinnerStatus();
 ...
 const handleSet1CloserSubmission = async (playMode: string, amount?: number) => {
  ...
  const { data: setlistData, error: setlistError } = await supabase
    .from('setlists')
    .select('set1_closer, show_opens_at, winner_decision_time')
  ...
  const actualSong = setlistData.set1_closer?.trim().toLowerCase();
  ...
  if (playMode === 'fun') {
    if (now >= showOpensAt) {
      ...
      storeSet1CloserWinnerStatus(isUserWinner);
    } else {
      setIsWinner(null);
      storeSet1CloserWinnerStatus(null);
    }
  } else {
    if (now >= winnerDecisionTime) {
      const status = actualSong === guessedSong;
      setIsWinner(status);
      storeSet1CloserWinnerStatus(status);
      isUserWinner = status;
    } else {
      const localStatus = getSet1CloserWinnerStatus();
  ...
  if (insertError) {
    ...
    alert('❌ There was a problem submitting your Set 1 Closer prediction.');
    return;
  }
...
/>
          ...
          <div className="mt-8 mb-8 perspective-1500 rotateX-12">
            <FourWaysToPlay
              ...
              onSubmissionClick={handleSet1CloserSubmission}
